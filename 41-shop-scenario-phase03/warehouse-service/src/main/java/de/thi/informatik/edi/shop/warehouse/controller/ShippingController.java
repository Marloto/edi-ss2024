...
import de.thi.informatik.edi.shop.warehouse.model.ShippingStatus;
...
public class ShippingController {
	...
	public List<ShippingResponse> list() {
		...
		warehouse.getShippings().forEach(el -> {
			if(el.getStatus() == ShippingStatus.PAYED || el.getStatus() == ShippingStatus.SHIPPED) {
				result.add(ShippingResponse.fromShipping(el));
			}
		});
		return result;
	}
